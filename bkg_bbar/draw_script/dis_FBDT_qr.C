...
void draw_hist(TH1D* hist,string address, string file, string variable, int flag=1){
    ...
    for(Int_t i=0; i < nentries; i++){
        ...
        if(flag == 1){
            if(qrMC == -1){
                //cout<<"flavor tag side B:"<<qrMC<<endl;
                hist->Fill(var);
            }
        ...
        //if(abs(qrMC) == 2) cout<<"ERROR in ROE"<<endl;
    }
...
void dis_FBDT_qr(){
    ...
    h_B0->SetMaximum(h_other->GetMaximum()*1.1);

    //h_sigMC->SetMaximum(10000000);
    
    TLegend *legend = new TLegend(0.25,0.70,0.45,0.84);
    ...
    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/plots/bkg_bbar/png/dis_FBDT_qr.png");
    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/plots/bkg_bbar/pdf/dis_FBDT_qr.pdf");
