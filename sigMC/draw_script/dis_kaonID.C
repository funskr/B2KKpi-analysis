...
void draw_hist(TH1D* hist,string address, string file, string variable, bool flag=true){
    ...
    //set variables
    string PID = variable + "_kaonID";
    string mcinfo = variable + "_isSignal";

    Double_t var;
    chain->SetBranchAddress(PID.c_str(), &var);

    Double_t isSignal;
    chain->SetBranchAddress(mcinfo.c_str(), &isSignal);
...
void dis_kaonID(){
    ...
    draw_hist(hsigMC,"/home/belle2/yuanmk/data/B2KKpi/sigMC/root/B2KKpi_without_kaonID.root","B0","Kp", true);
    draw_hist(hsigMC,"/home/belle2/yuanmk/data/B2KKpi/sigMC/root/B2KKpi_without_kaonID.root","B0","Km", true);

    TH1D *hmisid=new TH1D("hmisid", "hmisid", xbins, xmin, xmax);
    hmisid->SetLineColor(kBlue);
    hmisid->SetLineWidth(2);
    draw_hist(hmisid,"/home/belle2/yuanmk/data/B2KKpi/sigMC/root/B2KKpi_without_kaonID.root","B0","Kp", false);
    draw_hist(hmisid,"/home/belle2/yuanmk/data/B2KKpi/sigMC/root/B2KKpi_without_kaonID.root","B0","Km", false);
    ...
    hmisid->Scale(1/hmisid->GetEntries());

    hsigMC->GetXaxis()->SetTitle("kaonID");
    ...
    TLegend *legend = new TLegend(0.66,0.60,0.86,0.84);
    ...
    TArrow *xy1 = new TArrow(0.2, 0, 0.2, hsigMC->GetMaximum()*0.3,0.015,"<");
    xy1->SetLineColor(kBlue);xy1->SetLineWidth(2);
    xy1->Draw();

    TArrow *xy2 = new TArrow(-0.2, 0, -0.2, 1e-3,0.015,"<");
    xy2->SetLineColor(kBlue);xy2->SetLineWidth(2);
    //xy2->Draw();

    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/sigMC/plots/png/dis_kaonID.png");
    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/sigMC/plots/pdf/dis_kaonID.pdf");
}