...
void draw_hist(TH1D* hist,string address, string file, bool flag=true){
    ...
    for(Int_t i=0; i < nentries; i++){
        ...
        if(gamma1_clusterReg!=ECL_REG && gamma2_clusterReg!=ECL_REG) continue;
        if(gamma1_clusterReg==ECL_REG && gamma2_clusterReg!=ECL_REG){
            if(flag){
                if(isSignal == 1){
...
void dis_E_barrel(){
    ...
    hsigMC->GetXaxis()->SetTitle("clusterE in Barrel(GeV)");
    ...
    TArrow *xy1 = new TArrow(0.06, 0, 0.06, hsigMC->GetMaximum()*0.3,0.015,"<");
    ...
    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/sigMC/plots/png/dis_E_barrel.png");
    mbc->SaveAs("/home/belle2/yuanmk/analysis/B2KKpi/sigMC/plots/pdf/dis_E_barrel.pdf");
}
