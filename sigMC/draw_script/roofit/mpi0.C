...
void mpi0()
{
   ...
   // ---------------------
    ...
    Double_t xmin = 0.08, xmax = 0.18;

    RooRealVar x("pi0_InvM", "pi0_InvM", xmin, xmax);
    RooRealVar mean("mean", "mean of gaussians", 0.13, 0.1, 0.15);
...
// Build  background PDF:
   ...
   chain->Add("/home/belle2/yuanmk/analysis/B2KKpi/sigMC/ana_B2KKpi_signalMC_pion_mass.root");

   
   TTree* tree= chain->CopyTree("pi0_isSignal==1");
   ...
   RooPlot *xframe = x.frame(Title("Fit to M_{#pi}"));
   data.plotOn(xframe);
   model.plotOn(xframe);
   ...
   TCanvas *c = new TCanvas("rf101_basics", "rf101_basics", 800, 600); //c->Divide(2);
   c->cd();
   gPad->SetLeftMargin(0.15);
   xframe->GetYaxis()->SetTitleOffset(1.6);
   xframe->GetXaxis()->SetTitle("M_{#pi}/(GeV/c^{2})");
   xframe->Draw();
    ...
    TArrow *xy1 = new TArrow(mean.getValV()-3.5*sigma.getValV(), 0, mean.getValV()-3.5*sigma.getValV(), xframe->GetMaximum()*0.2,0.015,"<");
    xy1->SetLineColor(kRed);xy1->SetLineWidth(2);
    xy1->Draw();

    TArrow *xy2 = new TArrow(mean.getValV()+3.5*sigma.getValV(), 0, mean.getValV()+3.5*sigma.getValV(), xframe->GetMaximum()*0.2,0.015,"<");
    xy2->SetLineColor(kRed);xy2->SetLineWidth(2);
    xy2->Draw();

    cout<<"pi0 constrain: "<<mean.getValV()-3.5*sigma.getValV()<<" < m_pi0 < "<<mean.getValV()+3.5*sigma.getValV()<<endl;
